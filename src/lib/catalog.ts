...
import { categoryDiscount, effectivePrice } from "./pricing";
...
/**
 * Re-price a validated item list against the DB — never trusts a
 * client-sent price, so a tampered request can't set its own numbers or
 * corrupt the revenue total. Shared by checkout and the admin order editor.
 */
