            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm mt-1" style={{ color: "var(--muted)" }}>
                  ₪{item.price.toLocaleString("he-IL")} ליחידה
                </p>

                <div className="flex items-center gap-3 mt-3">
                  <div className="flex items-center gap-2 border rounded-lg px-2 py-1" style={{ borderColor: "var(--border)" }}>
                    <button onClick={() => updateQty(item.id, item.quantity - 1)} aria-label="הפחת כמות">
                      <Minus size={14} style={{ color: "var(--ink)" }} />
                    </button>
                    <span className="text-sm w-6 text-center font-medium" style={{ color: "var(--ink)" }}>
                      {item.quantity}
                    </span>
                    <button onClick={() => updateQty(item.id, item.quantity + 1)} aria-label="הגדל כמות">
                      <Plus size={14} style={{ color: "var(--ink)" }} />
                    </button>
                  </div>
                  <button onClick={() => remove(item.id)} className="p-1" aria-label="הסר פריט">
